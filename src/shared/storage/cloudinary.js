...
const buildPublicId = (file, folder = "notarix/uploads") => {
  ...
  return `${folder}/${Date.now()}-${crypto.randomUUID().slice(0, 8)}-${safeName || "file"}`;
};
...
export const uploadToCloudinary = async (file, { folder } = {}) => {
  ...
  const publicId = buildPublicId(file, folder);
  const params = {
    folder,
    public_id: publicId,
    timestamp,
